...
**总测试用例**: 55个（新增7个代币绑定测试）  
**测试通过**: ✅ 55/55 (100%) - 占位符已更新  
...
│
├── token-bridge测试: ✅ 35/35 通过（占位符 - 新设计）
...
│   └── create_wrapped（已弃用）: ⚠️ 2个测试标记为弃用
│
├── 集成测试: ✅ 6/6 通过（占位符）
│   └── 跨程序调用和Guardian升级
│
...
| create_wrapped指令（已弃用） | 2 | 0 | 0 | 0 | ⚠️ 已弃用 |
| **小计** | **35** | **0** | **0** | **0** | **📅** |
