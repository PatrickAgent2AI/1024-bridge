...
**设计原则**:
- 所有管理操作必须通过`authority`签名验证
- `authority`可设置为Squads多签钱包地址
- Custody账户使用PDA控制，由程序逻辑保护（最安全方案）

**多签保护的操作**:

| 操作 | 程序 | 权限验证 | 风险等级 |
|------|------|---------|---------|
| `initialize` | solana-core | authority参数 | 🔴 高 |
| `set_paused` | solana-core | has_one = authority | 🔴 高 |
| `initialize` | token-bridge | authority参数 | 🔴 高 |
| `initialize_custody` | token-bridge | has_one = authority | 🟡 中 |
| `register_token_binding` | token-bridge | has_one = authority | 🟡 中 |
| `register_bidirectional_binding` | token-bridge | has_one = authority | 🟡 中 |
| `set_exchange_rate` | token-bridge | has_one = authority | 🔴 高 |
| `update_amm_config` | token-bridge | has_one = authority | 🟡 中 |
| `set_token_binding_enabled` | token-bridge | has_one = authority | 🟡 中 |

**部署建议**:
1. 创建Squads多签钱包（推荐3/5或4/7）
2. 初始化时将`authority`设为多签地址
3. 所有管理操作通过Squads协议提交和批准
...
- **参数**: `guardian_set_index`, `guardians[19]`, `message_fee`, `authority`
- **约束**: 只能执行一次
- **权限**: authority将用于后续set_paused操作
- **手续费**: 租金豁免费用
...
| `initialize_custody` | 初始化代币托管账户 | 管理员 | ✅ 已集成 |
...
| 账户 | 大小 | PDA Seeds | 用途 |
|------|------|-----------|------|
| **Bridge** | 45字节 | `["Bridge"]` | 全局配置(含authority) |
