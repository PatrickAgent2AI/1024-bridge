...
export interface TokenTransferPayload {
  ...
  // 新增兑换字段
  targetToken?: Buffer;       // 32 bytes 目标链代币地址（可选，用于兑换）
...
/**
 * 序列化TokenTransfer Payload（支持新旧版本）
 */
export function serializeTokenTransferPayload(payload: TokenTransferPayload): Buffer {
  // 检测是否是新版本（包含兑换字段）
  const isNewVersion = payload.targetToken !== undefined;
  const bufferSize = isNewVersion ? 133 : 77;
  ...
  if (isNewVersion) {
    ...
    // targetAmount: uint64
    const targetAmountHex = (payload.targetAmount || BigInt(0)).toString(16).padStart(16, '0');
    Buffer.from(targetAmountHex, 'hex').copy(buffer, offset);
    offset += 8;
    
    // exchangeRateNum: uint64
    const rateNumHex = (payload.exchangeRateNum || BigInt(1)).toString(16).padStart(16, '0');
    Buffer.from(rateNumHex, 'hex').copy(buffer, offset);
    offset += 8;
    
    // exchangeRateDenom: uint64
    const rateDenomHex = (payload.exchangeRateDenom || BigInt(1)).toString(16).padStart(16, '0');
    Buffer.from(rateDenomHex, 'hex').copy(buffer, offset);
  }
