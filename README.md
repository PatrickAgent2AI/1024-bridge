...
| **Wrapped Token** | 目标链铸造的源链代币映射（非原生代币），如Ethereum USDC → Solana wrappedUSDC |
...
### guardian/ - Guardian验证器
- **config/**: 链配置管理（支持动态配置EVM/SVM链）
- **p2p/**: libp2p网络，Guardian间签名聚合
- **watcher/**: 监听链上事件（LogMessagePublished等）
- **signer/**: 对观察到的消息进行ECDSA签名
- **api/**: 对外REST API，提供VAA查询、状态监控
- **storage/**: PostgreSQL存储VAA，Redis缓存签名
- **types/**: 核心数据结构（VAA、Guardian Set、链事件）
